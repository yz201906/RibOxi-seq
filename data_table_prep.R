...
count_data <- final_counts%>%unite(chr, base, gene)%>%dplyr::select(chr, starts_with(args[1]), starts_with(args[2])) %>% as.data.frame()
...
rm(Control_num)
rm(Treatment_num)
rm(normalized_counts, count_data, dds, col_data)
